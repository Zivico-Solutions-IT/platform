...
const viewedNewUsersStorageKey = 'novafxm-admin-viewed-new-users';
      ...
      <Svg height="100%" width="100%" viewBox={`0 0 500 ${height}`} style={{ overflow: 'visible' }}>
        ...
        {hovered ? (
          <G pointerEvents="none">
            <Line x1={hovered.point1.x} y1={paddingY} x2={hovered.point1.x} y2={chartHeight - paddingY} stroke={colors.muted} strokeWidth={1} strokeDasharray="3 3" opacity={0.65} />
            <Circle cx={hovered.point1.x} cy={hovered.point1.y} r={5.5} fill={color1} stroke={colors.panel} strokeWidth={2} />
            <Circle cx={hovered.point2.x} cy={hovered.point2.y} r={5.5} fill={color2} stroke={colors.panel} strokeWidth={2} />
            <Rect x={Math.min(chartWidth - 150, Math.max(6, hovered.point1.x - 70))} y={6} width={140} height={42} rx={7} fill={colors.panel} stroke={colors.border} strokeWidth={1} />
            <SvgText x={Math.min(chartWidth - 80, Math.max(76, hovered.point1.x))} y={18} fontSize="9" fontWeight="700" fill={colors.text} textAnchor="middle">{hovered.label}</SvgText>
            <SvgText x={Math.min(chartWidth - 80, Math.max(76, hovered.point1.x))} y={30} fontSize="9" fontWeight="700" fill={color1} textAnchor="middle">{label1}: {hovered.point1.value}</SvgText>
            <SvgText x={Math.min(chartWidth - 80, Math.max(76, hovered.point1.x))} y={41} fontSize="9" fontWeight="700" fill={color2} textAnchor="middle">{label2}: {hovered.point2.value}</SvgText>
          </G>
        ) : null}
      </Svg>
    </View>
  ...
  return (
    <View style={{ height, width: '100%', overflow: 'visible' }}>
      ...
      <Svg height="100%" width="100%" viewBox={`0 0 500 ${height}`} style={{ overflow: 'visible' }}>
        ...
        {hovered ? (
          <G pointerEvents="none">
            <Line x1={hovered.x + hovered.w / 2} y1={paddingY} x2={hovered.x + hovered.w / 2} y2={chartHeight - paddingY} stroke={color} strokeWidth={1} strokeDasharray="3 3" opacity={0.55} />
            <Rect x={Math.min(chartWidth - 130, Math.max(6, hovered.x + hovered.w / 2 - 60))} y={6} width={120} height={30} rx={7} fill={colors.panel} stroke={color} strokeWidth={1} />
            <SvgText x={Math.min(chartWidth - 70, Math.max(66, hovered.x + hovered.w / 2))} y={18} fontSize="9" fontWeight="700" fill={colors.text} textAnchor="middle">{hovered.label}</SvgText>
            <SvgText x={Math.min(chartWidth - 70, Math.max(66, hovered.x + hovered.w / 2))} y={29} fontSize="9" fontWeight="700" fill={color} textAnchor="middle">{hovered.value}</SvgText>
          </G>
        ) : null}
      </Svg>
    </View>
  ...
  return (
    <View style={{ height, width: '100%', overflow: 'visible' }}>
      ...
      <Svg height="100%" width="100%" viewBox={`0 0 500 ${height}`} style={{ overflow: 'visible' }}>
        ...
        {hovered ? (
          <G pointerEvents="none">
            <Line x1={hovered.groupX} y1={paddingY} x2={hovered.groupX} y2={chartHeight - paddingY} stroke={colors.muted} strokeWidth={1} strokeDasharray="3 3" opacity={0.65} />
            <Rect x={Math.min(chartWidth - 150, Math.max(6, hovered.groupX - 70))} y={6} width={140} height={42} rx={7} fill={colors.panel} stroke={colors.border} strokeWidth={1} />
            <SvgText x={Math.min(chartWidth - 80, Math.max(76, hovered.groupX))} y={18} fontSize="9" fontWeight="700" fill={colors.text} textAnchor="middle">{hovered.label}</SvgText>
            <SvgText x={Math.min(chartWidth - 80, Math.max(76, hovered.groupX))} y={30} fontSize="9" fontWeight="700" fill={color1} textAnchor="middle">{label1}: {hovered.value1}</SvgText>
            <SvgText x={Math.min(chartWidth - 80, Math.max(76, hovered.groupX))} y={41} fontSize="9" fontWeight="700" fill={color2} textAnchor="middle">{label2}: {hovered.value2}</SvgText>
          </G>
        ) : null}
      </Svg>
    </View>
...
export default function AdminScreen({ initialSection, hideSidebar = false }) {
  ...
  const renderRegistrationCode = () => (
    <View className="rounded-2xl border p-4 md:p-6" style={{ backgroundColor: colors.panel, borderColor: colors.border }}>
      ...
      <Text className="mt-1 max-w-3xl text-sm" style={{ color: colors.muted }}>When a code is active, every new public NovaFXM registration must enter it. Deleting it reopens normal public registration without a code.</Text>
  ...
  const filteredDepositsAndWithdrawals = useMemo(() => {
    ...
    const deposits = (data.deposits || []).filter((d) => (d.status === 'approved' || d.status === 'completed' || d.status === 'pending') && filterFn(d));
