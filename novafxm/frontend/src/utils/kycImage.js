...
/** Converts large camera photos to a review-quality upload below the API request limit. */
export async function kycImageDataUrl(file) {
  if (!file) throw new Error('Choose an image first.');
  if (!String(file.type || '').startsWith('image/')) throw new Error('Use a JPG, PNG, or WEBP image.');
  if (typeof document === 'undefined' || typeof Image === 'undefined' || typeof URL === 'undefined') return readFileDataUrl(file);
