...
export default function AgentManagement() {
  ...
  useEffect(() => {
    AsyncStorage.getItem('novafxm.staff-role-templates.v1').then((saved) => {
  ...
  const saveRoleTemplates = async () => {
    try {
      await AsyncStorage.setItem('novafxm.staff-role-templates.v1', JSON.stringify(roleTemplates));
