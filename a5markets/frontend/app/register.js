...
export default function RegisterScreen() {
  ...
  const phoneValidationError = (phone, country) => {
    ...
    if (phoneDigits.length < 4 || phoneDigits.length > 14) return 'Enter a valid phone number.';

    let parsedPhone;
    try {
      parsedPhone = parsePhoneNumberFromString(`${country.dialCode}${trimmedPhone}`, country.code);
    } catch {
      parsedPhone = null;
    }
  ...
  const handlePhoneChange = (text) => {
    ...
    const country = matchedCountry || selectedCountry;
    const phone = country && cleanedPhone.startsWith(country.dialCode)
      ? cleanedPhone.slice(country.dialCode.length).trimStart()
          ...
          <View>
              ...
              <View>
                <View className="flex-row items-center rounded-full border" style={inputStyle}>
                  <TouchableOpacity
                    ...
                    className="flex-row items-center gap-1 border-r px-4 py-3"
            ...
            <View className="mb-4">
              <View className="relative">
                <TextInput
                  className="rounded-full border px-5 py-2.5 pr-12 text-xs"
            ...
            <Pressable
              onPress={submit}
              disabled={loading}
              className="w-full rounded-full bg-[#064681] py-2.5 items-center"
              style={{ opacity: loading ? 0.7 : 1 }}
            >
              <Text className="text-white font-semibold text-xs">{loading ? 'Signing up...' : 'SIGN UP'}</Text>
            </Pressable>
          </View>
