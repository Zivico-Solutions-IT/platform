                ...
                {visibleAccounts.map((account) => {
                  ...
                  return (
                    <View key={account.id} className="rounded-2xl border p-2" style={{ backgroundColor: account.type === 'Live' ? `${colors.primary}10` : colors.surface, borderColor: account.type === 'Live' ? colors.primary : colors.border }}>
                      <View className="flex-row items-center justify-between">
                        <View className="min-w-0 flex-1"><Text className="text-xs font-semibold" numberOfLines={1} style={{ color: colors.text }}>{account.name}</Text>{account.type === 'Live' ? <Text className="mt-0.5 text-[9px] font-bold uppercase" style={{ color: colors.primary }}>Live account</Text> : null}</View>
                    ...
                    return (
                      <View key={account.id} className={`flex-row ${accountIndex > 0 ? 'border-t' : ''}`} style={{ borderColor: colors.border }}>
                        <View style={{ width: 320, backgroundColor: account.type === 'Live' ? `${colors.primary}0D` : 'transparent' }} className="px-3 py-4">
                          <View className="flex-row items-center justify-between"><Text className="text-sm font-semimedium" style={{ color: colors.text }}>{account.name}</Text>{account.type === 'Live' ? <Text className="rounded-full px-2 py-1 text-[9px] font-bold uppercase" style={{ backgroundColor: `${colors.primary}20`, color: colors.primary }}>Live</Text> : null}</View>
