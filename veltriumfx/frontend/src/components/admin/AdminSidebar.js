  ...
  const renderConsoleHeader = (compact = false, showClose = false) => {
    ...
    const openNovaMaster = async () => {
      if (Platform.OS === 'web' && typeof window !== 'undefined') {
        ...
        const configuredUrl = String(process.env.EXPO_PUBLIC_NOVAFXM_MASTER_URL || 'https://platform.novafxm.com').replace(/\/(login|master)\/?$/, '');
        const target = local
      ...
      {adminUser?.role === 'master' ? (
        <View className="mt-4">
          <Pressable onPress={() => setPlatformMenuOpen((open) => !open)} className="flex-row items-center rounded-xl border px-3 py-2.5" style={{ backgroundColor: `${colors.primary}12`, borderColor: `${colors.primary}40` }}>
            ...
            <Text className="ml-2 flex-1 text-sm font-semibold" style={{ color: colors.text }}>VeltriumFX Master</Text>
