...
export default function MasterDashboard() {
  ...
  useEffect(() => { setSelectedAdminId(selectedProject?.admins?.[0]?.id || null); setExpandedAdminId(null); }, [selectedProjectId]);
  ...
  const selectProject = async () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      await AsyncStorage.setItem('x-project-id', String(selectedProject.id));
      await AsyncStorage.setItem('x-project-name', selectedProject.name);
    }
        ...
        {mobile ? (
          <View className="mb-5 flex-row gap-2">
            ...
            <Pressable onPress={() => setTab('symbols')} className="flex-1 rounded-xl py-3" style={{ backgroundColor: tab === 'symbols' ? colors.primary : colors.surface }}>
              <Text className="text-center font-semibold" style={{ color: tab === 'symbols' ? '#0B0B0B' : colors.text }}>Symbol Settings</Text>
            </Pressable>
