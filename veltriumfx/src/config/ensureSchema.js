...
async function ensureSchema() {
  ...
  // Seed VeltriumFX as the default project for this platform instance
  // NOTE: Both novafxm and veltriumfx backends share the same DB.
  // The novafxm backend handles seeding 'novafxm' project; here we just ensure 'veltriumfx' exists.
  const [projects] = await queryInterface.sequelize.query('SELECT id FROM projects WHERE identifier = "veltriumfx"');
  ...
  if (defaultProjectId) {
    // Only migrate orphaned records (no project_id) to the veltriumfx default project if novafxm doesn't exist yet
    const [novaProjects] = await queryInterface.sequelize.query('SELECT id FROM projects WHERE identifier = "novafxm"');
    if (novaProjects.length === 0) {
      for (const table of tables) {
  ...
  // Ensure default VeltriumFX Admin exists in veltriumfx_db with full permissions
  ...
  const [vtAdmins] = await queryInterface.sequelize.query('SELECT id FROM users WHERE email = "admin@veltriumfx.com"');
  if (vtAdmins.length === 0) {
    ...
    await queryInterface.sequelize.query(
      `INSERT INTO users (name, email, password, role, project_id, permissions, account_type, leverage, trading_level, trading_status, verification_status, created_at, updated_at)
       VALUES ('VeltriumFX Admin', 'admin@veltriumfx.com', '${vtAdminHash}', 'admin', ${defaultProjectId}, '${allPermissionsJson}', 'Live', 500, 'Standard', 'active', 'unverified', NOW(), NOW())`
    );
    const [newVtAdmin] = await queryInterface.sequelize.query('SELECT id FROM users WHERE email = "admin@veltriumfx.com"');
    if (newVtAdmin.length > 0) {
      await queryInterface.sequelize.query(
        `INSERT IGNORE INTO wallets (user_id, project_id, balance, equity, free_funds, bonus, margin, margin_level, created_at, updated_at)
         VALUES (${newVtAdmin[0].id}, ${defaultProjectId}, 0, 0, 0, 0, 0, 0, NOW(), NOW())`
      );
    }
  } else {
    // Ensure existing admin has full permissions set
    await queryInterface.sequelize.query(
      `UPDATE users SET permissions = '${allPermissionsJson}' WHERE email = "admin@veltriumfx.com" AND (permissions IS NULL OR permissions = '[]')`
    );
  }
